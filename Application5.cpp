...
int Application5::Initialize()
{
	...
	for (int i = 0; i <= AAKERNEL_SIZE; i++) {
		...
		*/
		...
		valueListOffset[0] = (GzPointer)&AAFilter[i][0];
		
		nameListOffset[1] = GZ_AASHIFTY;
		valueListOffset[1] = (GzPointer)&AAFilter[i][1];

		float zero = 0.0;
		if (i == AAKERNEL_SIZE) {
			valueListOffset[0] = (GzPointer)&zero;
			valueListOffset[1] = (GzPointer)&zero;
		}
...
int Application5::Render()
{
	...
	for (int i = 0; i < m_nWidth * m_nHeight; i++) {
		...
		for (int j = 0; j < AAKERNEL_SIZE; j++) {
			pix.red += (float)m_pRender[j]->pixelbuffer[i].red * AAFilter[j][2];
			pix.green += (float)m_pRender[j]->pixelbuffer[i].green * AAFilter[j][2];
			pix.blue += (float)m_pRender[j]->pixelbuffer[i].blue * AAFilter[j][2];
		}
		
		m_pRender[AAKERNEL_SIZE]->pixelbuffer[i] = pix;
	}
